...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0 <= s <= 1
    assert_close(1 - s, minitorch.operators.sigmoid(-a))
    if a == 0:
        assert s == 0.5
    eps = 1e-6
...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    assert_close(minitorch.operators.mul(a, b), minitorch.operators.mul(b, a))
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert_close(
        minitorch.operators.mul(a + b, c),
        minitorch.operators.mul(a, c) + minitorch.operators.mul(b, c),
    )
